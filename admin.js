...
el("fillChineseBtn").onclick=()=>{let filled=0;Object.values(lessons).forEach(groups=>Object.values(groups).forEach(items=>items.forEach(entry=>{if(!entry.chinese&&CHINESE_TRANSLATIONS[entry.english]){entry.chinese=CHINESE_TRANSLATIONS[entry.english];filled++}})));saveLocal(filled?`已自动补上 ${filled} 条中文`:'现有资料的中文已经全部补齐');renderAll()};
